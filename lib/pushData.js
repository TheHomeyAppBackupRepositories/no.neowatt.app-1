...
async function push(self) {
    ...
    // Push data to NW API, chunked
    const chunkSize = 50;

    for (let i = 0; i < devices.length; i += chunkSize) {
        // Deler opp arrayet i mindre chunks
        const chunk = devices.slice(i, i + chunkSize);

        // Gjør POST-forespørselen med hver chunk
        try {
            ...
            console.log(`Chunk #${i} response` , res.ok, res.status);

            if (res.status !== 200) {
                throw new Error(`Push failed with status ${res.status}`);
            }
        } catch (error) {
            //console.error(`Error in sending chunk #${i}: `, error);
            // Abort further chunks if error is detected, wait until next push interval
            throw error;
        }
